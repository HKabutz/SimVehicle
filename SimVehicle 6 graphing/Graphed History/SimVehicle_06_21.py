...
if __name__ == '__main__':
    ...
    plt.plot(sim1.temp_p_hist_x, sim1.temp_p_hist_y,"r-",label='Simulated controller at '+str(sim1.vel)+'m/s',linewidth=2)
    plt.plot(a_GPS_PosLog[:, 1] / 100 - 2.7, a_GPS_PosLog[:, 2] / 100 - 1.8, "m.", markersize=4, label="Actual Path")
    ...
    plt.plot(sim1.temp_p_hist_x[4:-1], sim1.temp_p_hist_steer[4:-1],"r-",label='Simulated controller at '+str(sim1.vel)+'m/s',linewidth=2)
    plt.plot(a_GPS_PosLog[:, 1] / 100 - 2.7, -a_ControllerLog[:,3], "m.", label="Actual Steering", markersize=4)
    ...
    plt.savefig('img_LateralControl_simulate_19.png', dpi=500, bbox_inches='tight')
    # plt.subplot(212)
