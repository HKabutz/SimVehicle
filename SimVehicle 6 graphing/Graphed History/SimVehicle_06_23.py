    ...
    def run(self,opt_par=[0.0,0.0,0.0]):
        ...
        while time < 30:
            ...
            if self.lateral_controller == "PureP":  # Lateral controller: Pure Pursuit
                ...
                # Vehicle Position (rear point)
                x_veh = car.position_front[0] + x_noise_temp
                y_veh = car.position_front[1] + y_noise_temp
                # Lookahead r_rels and angle
                look_min = 0.1  # 5  # 0.1
                look_max = 4  # 20
                look_angle = 1.4
...
if __name__ == '__main__':
    ...
    sim1.setProperties(xx=1.0, yy=-0.40, aa=-0.40, set_vel=1.300)
    # sim1.run(opt_par=Opt_CFollow.x)
    # sim1.run(opt_par=[0.42296772, 0.36308098, 0.11522529])
    sim1.run(opt_par=[0, 0.6, 0.2])

    day = '2019-10-08-12-57-37'
    pd_SteeringLog = pd.read_csv('SoftTarget_SteeringActuator_'+day+'.txt')
    a_SteeringLog = np.array(pd_SteeringLog)
    row_SteeringLog, col_SteeringLog = np.shape(a_SteeringLog)
    print("SteeringLog size: ", row_SteeringLog)
    pd_GPS_PosLog = pd.read_csv('SoftTarget_PosLog_'+day+'.txt')
    a_GPS_PosLog = np.array(pd_GPS_PosLog)
    row_GPS_PosLog, col_GPS_PosLog = np.shape(a_GPS_PosLog)
    print("PosLog size: ", row_GPS_PosLog)
    pd_ControllerLog = pd.read_csv('SoftTarget_ControllerLog_'+day+'.txt')
    ...
    plt.figure(1)
    plt.subplot(211)
    plt.plot(path_x, path_y,"b--",label='Desired Path',linewidth=2)
    plt.plot(sim1.temp_p_hist_x[2:-1] - 1, sim1.temp_p_hist_y[2:-1],"r-",label='Simulated controller at '+str(sim1.vel)+'m/s',linewidth=2)
    plt.plot(a_GPS_PosLog[:, 1] / 100 - 5, a_GPS_PosLog[:, 2] / 100 - 1.8, "m.", markersize=5, label="Actual Path")
    #  plt.plot(sim1.MPC_t_x1,sim1.MPC_t_y1)
    plt.xlabel("Horizontal position [m]", fontsize=9)
    plt.ylabel("Lateral position [m]", fontsize=9)
    plt.title('Path following of straight line', fontsize=10) # 'Double Lange Change Maneuver'
    ...
    plt.plot(sim1.temp_p_hist_x[2:-1] - 1, sim1.temp_p_hist_steer[2:-1],"r-",label='Simulated controller at '+str(sim1.vel)+'m/s',linewidth=2)
    plt.plot(a_GPS_PosLog[:, 1] / 100 - 5, -a_ControllerLog[:,3], "m.", label="Actual Steering", markersize=5)
    # plt.plot((a_GPS_PosLog[:, 1] - a_GPS_PosLog[0, 1]) / 100, -a_ControllerLog[:, 8] * 57.29
    #         - a_ControllerLog[:, 7] * 57.29
    #         - a_ControllerLog[:, 6] * 57.29
    #         - a_ControllerLog[:, 5] * 57.29, "m.", label="Actual Steering", markersize=4)
    plt.xlabel("Distance traveled [m]", fontsize=9)
    plt.ylabel("Steering angle in degrees", fontsize=9)
    plt.title('Steering angle', fontsize=10)
    plt.axis([0,x_length,-21,21])
    plt.xticks(fontsize=8)
    plt.yticks(fontsize=8)
    plt.legend(loc='lower right', fontsize=8)
    ...
    plt.savefig('img_LateralControl_simulate_23.png', dpi=500, bbox_inches='tight')
    # plt.subplot(212)
    ...
    # plt.plot(sim1.temp_p_hist_x, sim1.temp_track_psi, "b-", label='psi')
    # plt.plot(sim1.temp_p_hist_x, sim1.temp_track_g, "g-", label='g')
    # plt.plot(sim1.temp_p_hist_x, sim1.temp_p_hist_steer, "m-",
    #          label=sim1.lateral_controller + ' at ' + str(sim1.vel) + 'm/s', linewidth=3)
